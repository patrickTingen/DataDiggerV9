...
/* <JAG> - Support AS400 (FC specifiek) */
define variable gcLogischeDbNaamDataSr  as character  no-undo initial ?.    /* Dataserver, bijvoorbeeld: AS400 */
define variable gcLogischeDbNaamSchema  as character  no-undo initial ?.
/* </JAG> - Support AS400 (FC specifiek) */
...
/* <JAG> - Support AS400 (FC specifiek) */
&IF DEFINED(EXCLUDE-getDatabaseName) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getDatabaseName Procedure 
FUNCTION getDatabaseName returns character
  ( input pcConvert       as character
  , input pcDatabaseName  as character
  ) FORWARD.
...
PROCEDURE checkForUpgrade :
  ...
  /* <JAG> - Support V9 (compile-time) - Dit werkt niet: copy-lob kent ie niet */
  /*
...
/*------------------------------------------------------------------------
 ...
 do iLoop = 1 to iMaxQueryHistory:
   cSetting = getRegistry ( substitute('DB:&1'      , getDatabaseName("TO-DATASR", pcDatabase) )  /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
...
------------------------------------------------------------------------------*/
  ...
  define output parameter plContinue as logical     no-undo initial yes.  /* <JAG> - Support V9 (compile-time) - </JAG> */
  ...
  /* <JAG> - Support V9 (compile-time) - Dit werkt niet: write-xml kent ie niet */
  /*
...
/*------------------------------------------------------------------------
  ...
  DEFINE VARIABLE cImport AS CHARACTER   NO-UNDO EXTENT 1000.   /* <JAG> - Support V9 (compile-time) - was 2800, moest lager anders klapt de sessie </JAG> */
  ...
  REPEAT:
    ...
    /* hTable:BUFFER-VALIDATE(). */     /* <JAG> - Support V9 (compile-time) - Dit werkt niet: buffer-validate kent ie niet </JAG> */
  END.
...
/*------------------------------------------------------------------------
  ...
  DEFINE OUTPUT PARAMETER plAscending AS LOGICAL     NO-UNDO initial yes.   /* <JAG> - Support V9 (compile-time) </JAG> */
  
  DEFINE VARIABLE hColumn AS HANDLE      NO-UNDO.
  DEFINE VARIABLE iColumn AS INTEGER     NO-UNDO.
  
  /* <JAG> - Support V9 (compile-time) - Dit werkt niet: sort-ascending kent ie niet */
  /*
...
/*------------------------------------------------------------------------
  ...
  */
  ...
  cSelectedFields = getRegistry( substitute('DB:&1'    , getDatabaseName("TO-DATASR", pcDatabase) )   /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
  ...
  /* Get field ordering */
  cFieldOrder = getRegistry( substitute('DB:&1'        , getDatabaseName("TO-DATASR", pcDatabase) )   /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
  ...
  repeat while not hQuery:query-off-end:
    ...
    do iFieldExtent = 0 to hBufferField:buffer-field('_Extent'):buffer-value:
      ...
      ttField.cFormat = getRegistry( substitute('DB:&1'       , getDatabaseName("TO-DATASR", pcDatabase) )  /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
      ...
      do:
        ttField.cFilterValue = getRegistry( substitute('DB:&1'       , getDatabaseName("TO-DATASR", pcDatabase) )   /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
...
/*------------------------------------------------------------------------
  ...
  define variable cShowDatabase as character  no-undo.
  define variable cDatabases    as character  no-undo.

  publish "timerCommand" ("start", "CollectTables").
  
  /* <JAG> - Support AS400 (FC specifiek) */
  if   gcLogischeDbNaamDataSr = ?
   and gcLogischeDbNaamSchema = ? then
  ...
  do iDatabase = 1 to num-dbs:
    ...
    /* <JAG> - Support AS400 (FC specifiek) */
    if dbtype(iDatabase) = "AS400" then
      next BLOCKGetTables.
    /* </JAG> - Support AS400 (FC specifiek) */
    ...
    do:
      ...
      repeat while not hQuery:query-off-end:
        cShowDatabase = getDatabaseName("TO-DATASR", cDatabase).  /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
        
        create ttTable.
        assign 
          ttTable.cDatabase     = cDatabase
          ttTable.cShowDatabase = cShowDatabase     /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
          /* <JAG> - Support V9 (compile-time) */
          ttTable.cTableName    = hBuffer:buffer-field('_file-name'):buffer-value
          ttTable.cTableDesc    = hBuffer:buffer-field('_desc'):buffer-value
          ttTable.lHidden       = hBuffer:buffer-field('_hidden'):buffer-value
          /* </JAG> - Support V9 (compile-time) */
          .
        hQuery:get-next().
      end.
  ...
  repeat:
    ...
    do:
      ...
      if available ttTable then
      do:
        assign ttTable.tLastUsed = date(entry(2,cLine,'=')) no-error.   /* <JAG> - Support V9 (compile-time) </JAG> */
      end.
    end. /* queriesServed */
    ...
    do:
      ...
      repeat:
        ...
        assign 
          ttTable.cDatabase  = cDatabase
          /* <JAG> - Support V9 (compile-time) */
          ttTable.cTableName = hFile:buffer-field('_file-name'):buffer-value
          ttTable.cTableDesc = hFile:buffer-field('_desc'):buffer-value
          ttTable.lHidden    = hFile:buffer-field('_hidden'):buffer-value
          /* </JAG> - Support V9 (compile-time) */
          .
      end.
    end. /* opened */
...
/*------------------------------------------------------------------------
  ...
  do iField = 1 to num-entries(pcFilterFields):
    hFilterField = widget-handle(entry(iField,pcFilterFields)).   /* <JAG> - Support V9 (compile-time) </JAG> */
  ...
  do iButton = num-entries(pcButtons) to 1 by -1:
    hButton = widget-handle(entry(iButton,pcButtons)).    /* <JAG> - Support V9 (compile-time) </JAG> */
  ...
  do iField = 1 to phBrowse:num-columns:
    ...
    hFilterField = widget-handle(entry(iFilter, pcFilterFields)).   /* <JAG> - Support V9 (compile-time) </JAG> */
...
/*------------------------------------------------------------------------
  ...
  do iColumn = 1 to phBrowse:num-columns:
    ...
    do:
      /* phBrowse:set-sort-arrow(iColumn, plAscending ). */   /* <JAG> - Support V9 (compile-time) - set-sort-arrow niet ondersteund </JAG> */
    ...
    do:
      /* phBrowse:set-sort-arrow(iColumn, ? ). */ /* erase existing arrow */  /* <JAG> - Support V9 (compile-time) - set-sort-arrow niet ondersteund </JAG> */
    end.
  end.
...
/*------------------------------------------------------------------------
  ...
  /* <JAG> - Support AS400 (FC specifiek) - Skip de schemaholder en temp-db */

  /* Special options */
  BLOCKGetDatabases:
  do iCount = 1 to num-dbs:
    if   gcLogischeDbNaamDataSr = ?
     and dbtype(iCount) = "AS400" then
      gcLogischeDbNaamDataSr = ldbname(iCount).
    
    if   gcLogischeDbNaamSchema = ?
     and ldbname(iCount) begins "schema" then
      gcLogischeDbNaamSchema = ldbname(iCount).
    
    if  dbtype(iCount) = "AS400"
     or can-do("temp-db", ldbname(iCount)) then   /* Skip de temp-db */
      next BLOCKGetDatabases.
  ...
  assign
    gcLogischeDbNaamDataSr  = (if gcLogischeDbNaamDataSr = ? then
                                 ""
                               else
                                 gcLogischeDbNaamDataSr)
    gcLogischeDbNaamSchema  = (if gcLogischeDbNaamSchema = ? or gcLogischeDbNaamDataSr = "" then
                                 ""
                               else
                                 gcLogischeDbNaamSchema).
  
  if   gcLogischeDbNaamDataSr <> ""
   and gcLogischeDbNaamSchema <> "" then
    cDatabaseList = replace(cDatabaseList, gcLogischeDbNaamSchema, gcLogischeDbNaamDataSr).
...
FUNCTION getImagePath returns character
  ...
  /* <JAG> - Support V9 (compile-time) - Aangepast, check ook specifieke 'V9' versies */
  cImagePath = substitute('&1Image\&2'
                         , getProgramDir()
                         , replace(   pcImage
                                   ,  "."
  ...
  /* </JAG> - Support V9 (compile-time) - Aangepast */
...
/*------------------------------------------------------------------------
  ...
  pcDatabaseFilter = getDatabaseName("TO-SCHEMA", pcDatabaseFilter).   /* <JAG> - Support AS400 (FC specifiek) - </JAG> */
...
FUNCTION getDatabaseName returns character
  ...
  if pcDatabaseName <> "" then
  do:
    if   pcConvert = "TO-SCHEMA"
     and pcDatabaseName = gcLogischeDbNaamDataSr then
      pcDatabaseName = gcLogischeDbNaamSchema.
  
    if   pcConvert = "TO-DATASR"
     and pcDatabaseName = gcLogischeDbNaamSchema then
      pcDatabaseName = gcLogischeDbNaamDataSr.
  end.
  
  return pcDatabaseName.
end FUNCTION.
