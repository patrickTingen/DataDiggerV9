...
DEFINE FRAME fr-dialog
     ficFileName AT Y 15 X 60 COLON-ALIGNED
     cbDateFormat AT Y 97 X 325 COLON-ALIGNED
     cbiFieldSelection AT Y 95 X 60 COLON-ALIGNED
     tbExportSchema AT Y 90 X 485
     btnClose AT Y 225 X 575
     btnDump AT Y 225 X 495
     btnOpenLastDumpDir AT Y 175 X 618
     btnViewLastDump AT Y 175 X 595
     ficMessage AT Y 175 X 10 NO-LABEL
     cbNumericFormat AT Y 75 X 325 COLON-ALIGNED
     tbUseCustomizedFormats AT Y 75 X 485
     tbDumpReadyView AT Y 229 X 325
     tbDumpReadyExplore AT Y 229 X 170
     tbDumpReadyClose AT Y 229 X 25
     tbMinimalSchema AT Y 105 X 485
     ficMessageNow AT Y 156 X 10 NO-LABEL
     btnChooseDumpFile AT Y 15 X 620
     cbDumpType AT Y 45 X 60 COLON-ALIGNED
     cbiRecordSelection AT Y 70 X 60 COLON-ALIGNED
     "Last dump" VIEW-AS TEXT
          SIZE-PIXELS 55 BY 13 AT Y 135 X 10
     "After the dump ..." VIEW-AS TEXT
          SIZE-PIXELS 85 BY 13 AT Y 209 X 10
     RECT-2 AT Y 5 X 5
     RECT-3 AT Y 141 X 5
     RECT-4 AT ROW 11.19 COL 2
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         SIZE-PIXELS 673 BY 290
         TITLE "Dump to..."
         DEFAULT-BUTTON btnDump CANCEL-BUTTON btnClose.

DEFINE FRAME infoFrame
     btAbort AT Y 70 X 60
...
------------------------------------------------------------------------------*/
  ...
  run setStatusMessage
    ( input {&now}          /* <JAG> - Support V9 (compile-time) - </JAG> */
    , input substitute
  ...
  do with frame {&frame-name}:
    ...
    run setStatusMessage( input {&now}, input cStatus ).    /* <JAG> - Support V9 (compile-time) - </JAG> */
...
------------------------------------------------------------------------------*/
  ...
  do with frame {&frame-name}:
    ...
    /* <JAG> - Support V9 (compile-time) - Niet ondersteund in V9 */
    /*
...
------------------------------------------------------------------------------*/
  ...
  do with frame {&frame-name}:
    ...
    run setStatusMessage
      ( input date(getRegistry("DumpAndLoad", "DumpActionTimeStamp"))     /* <JAG> - Support V9 (compile-time) - </JAG> */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  define input parameter pidtTimeStamp as {&datetime-v} no-undo.
  ...
  do with frame {&frame-name}:
    ...
    if pidtTimeStamp = ? then
      ...
      ficMessageNow = string(pidtTimeStamp, {&datetime-f}).   /* <JAG> - Support V9 (compile-time) - </JAG> */
