...
DEFINE FRAME Dialog-Frame
     fiLogicalName AT Y 60 X 384 COLON-ALIGNED
     btnNew AT Y 20 X 314
     fiDatabaseName AT Y 110 X 384 COLON-ALIGNED
     edParameters AT Y 150 X 309 NO-LABEL
     btnConnect AT Y 280 X 10
     btnDisconnect AT Y 280 X 95
     btnCancel AT Y 280 X 610
     btnTest AT Y 20 X 519
     btnDelete AT Y 20 X 464
     btnEdit AT Y 20 X 414
     btnCopy AT Y 20 X 364
     fiDescription AT Y 85 X 384 COLON-ALIGNED
     btnSave AT Y 20 X 579
     btnUndo AT Y 20 X 629
     brConnections AT Y 5 X 0
     "Connection Details" VIEW-AS TEXT
          SIZE-PIXELS 95 BY 13 AT Y 0 X 309
     "Connection Parameters:" VIEW-AS TEXT
          SIZE-PIXELS 120 BY 13 AT Y 135 X 309
     RECT-3 AT Y 5 X 304
     RECT-20 AT Y 15 X 309
...
/*------------------------------------------------------------------------
  ...
  do with frame {&frame-name}:
    ...
    if cSetting <> ? then
    do:
      /* brConnections:set-sort-arrow(integer(entry(1,cSetting)), logical(entry(2,cSetting)) ). */ /* <JAG> - Support V9 (compile-time) - set-sort-arrow niet ondersteund </JAG> */
    end.
  end.
